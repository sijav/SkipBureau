...
export const caseOf = (dataFile: string): Case => {
  ...
  if (!country || !document) throw new PublishError(`${dataFile} is not a research case: give src/rules/research/<country>/<case>.ts, from apps/api.`)
  return {
    country,
    document,
 ...
 */
export const commitBytes = (repo: string, start: string, branch: string, files: readonly { path: string; bytes: Buffer }[], message: string): string => {
...
const isRules = (value: unknown): value is ResearchRules =>
  isRecord(value) && typeof value['country'] === 'string' && typeof value['research'] === 'string' && Array.isArray(value['versions']) && Array.isArray(value['regions'])
...
type Reader = { to: string; statuses?: string[]; nationality?: string; situation?: string; regions?: string[] }

const readerFor = (rules: ResearchRules, version: ResearchVersion): Reader => {
  ...
  // A version with no place, whose obligation has a place's version, is asked with a place none names.
  const named = new Set(
    rules.versions
      .filter((other) => other.obligation === version.obligation)
      .flatMap((other) => other.criteria.filter((criterion) => criterion.dimension === 'residenceRegion').map((criterion) => criterion.value)),
  )
  const elsewhere = rules.regions.find((region) => region.parent === null && !named.has(region.code))
...
const widerOf = (rules: ResearchRules, version: ResearchVersion): ResearchVersion[] => {
  ...
  return rules.versions.filter(
    (other) =>
      ...
      other.criteria.every((criterion) => version.criteria.some((mine) => mine.dimension === criterion.dimension && mine.value === criterion.value)),
  )
}

type Served = { key: string; operator: string; numericValue: string | null; textValue: string | null; unit: string | null; currency: string | null; sourceUrl: string; sourceName: string; verifiedAt: string }
...
const ask = async (query: string, variables: Record<string, unknown>): Promise<{ data: unknown; errors: string[] }> => {
  const response = await fetch(LIVE, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ query, variables }) })
  const parsed: unknown = await response.json().catch(() => null)
  const errors = isRecord(parsed) && Array.isArray(parsed['errors']) ? parsed['errors'].map((error: unknown) => (isRecord(error) && typeof error['message'] === 'string' ? error['message'] : String(error))) : []
  return { data: isRecord(parsed) ? parsed['data'] : undefined, errors: response.ok || errors.length > 0 ? errors : [`HTTP ${response.status}`] }
}
...
const main = async (): Promise<void> => {
  ...
  if (given.length !== 1 || !dataFile) throw new PublishError('give one data file: npm run research:publish -w @skipbureau/api -- [--down] src/rules/research/<country>/<case>.ts')
  ...
  if (!existsSync(resolve(API, the.agreed))) throw new PublishError(`${the.agreed} does not exist: a case is published from its agreed document.`)

  let reverts: string | undefined
  if (down) {
    const log = git(['log', `--format=%H%x00%B%x1e`, `--grep=^Research-Case: ${the.name}$`], { cwd: repo })
    const last = lastUnreverted(log, the.name)
    if (!last) throw new PublishError(`${the.name} has no publish that has not been taken down.`)
    reverts = last.id
  ...
  if (types.status !== 0) throw new PublishError(`the type check failed:\n${(types.stdout + types.stderr).trim().split('\n').slice(0, 20).join('\n')}`)
  ...
  if (!rules || !isCase(current)) throw new PublishError(`${the.countryFile} or ${the.dataFile} does not export what a research case exports.`)
  const digest = digestOf(rules)
  const previous = await caseAt(repo, start, the)

  const id = reverts ?? randomUUID()
  const commit = commitBytes(repo, start, branch, snapshot.map((file) => ({ path: inRepo(file.path), bytes: file.bytes })), messageOf(down ? 'down' : 'publish', the.name, id))
  const pushed = run('git', ['push', 'origin', branch.replace(/^refs\/heads\//, '')], { cwd: repo })
  if (pushed.status !== 0) throw new PublishError(`the push was refused: ${pushed.stderr.trim()}. ${commit} is on ${branch} and not on origin.`)
  ...
  while (deployed !== digest) {
    if (Date.now() > deadline) {
      throw new PublishError(
        `the deployed digest for ${rules.research} is ${deployed ?? 'missing'} after 15 minutes, not ${digest}: the deploy did not arrive or its load failed; a failed migration stops later deploys at P3009 until prisma migrate resolve runs on the deployed database.`,
      )
    }
  ...
  for (const version of current.versions ?? []) {
    ...
    const expected = [...version.facts, ...widerOf(rules, version).flatMap((wider) => wider.facts.filter((fact) => !version.facts.some((mine) => mine.key === fact.key)))]
    const missing = expected.filter((fact) => !shows(rules.sources, served, fact)).map((fact) => fact.key)
    if (errors.length > 0 || missing.length > 0) problems.push(`${identity(version)}: ${[...errors, ...missing.map((key) => `${key} not served`)].join('; ')}`)
  }
